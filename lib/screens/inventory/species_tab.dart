...
class SpeciesTab extends StatefulWidget {
  ...
  const SpeciesTab({super.key, required this.inventory, required this.speciesDao, required this.inventoryDao});
...
class _SpeciesTabState extends State<SpeciesTab> with AutomaticKeepAliveClientMixin {
  late SpeciesSortField _sortOption =
      widget.inventory.type == InventoryType.invTransectDetection ||
          ...
          ? SpeciesSortField.time
          : SpeciesSortField.name;
  ...
  bool get _allowsDuplicatedSpeciesNames =>
      widget.inventory.type == InventoryType.invTransectDetection ||
      widget.inventory.type == InventoryType.invPointDetection;

  Set<String> _existingSpeciesNamesExcluding(Species species) {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return _buildSpeciesList(widget.speciesDao, widget.inventoryDao);
  }
  ...
  Future<void> _updateSpeciesList(String inventoryId) async {
    ...
    debugPrint(
      '[SPECIES_TAB] Species list reloaded for inventory $inventoryId with ${widget.inventory.speciesList.length} species',
    );
  }
  ...
  Future<void> _deleteSpecies(Species species) async {
    ...
    final shouldAskAboutOtherInventories =
        !widget.inventory.isFinished &&
  ...
  Future<void> _deleteSpeciesFromOtherActiveInventories(Species species, SpeciesProvider speciesProvider) async {
    final inventoryProvider = Provider.of<InventoryProvider>(context, listen: false);
    for (final inventory in inventoryProvider.activeInventories) {
      if (inventory.id != species.inventoryId &&
          ...
          (inventory.type != InventoryType.invBanding &&
              inventory.type != InventoryType.invTransectDetection &&
              inventory.type != InventoryType.invPointDetection)) {
        await speciesProvider.removeSpeciesFromInventory(context, inventory.id, species.name);
  ...
  Future<bool> _showDeleteFromOtherListsConfirmationDialog(BuildContext context, String speciesName) async {
    return await showDialog<bool>(
          context: context,
          builder: (BuildContext context) {
            return AlertDialog.adaptive(
  ...
  Widget _buildSpeciesList(SpeciesDao speciesDao, InventoryDao inventoryDao) {
    return Column(
      children: [
        Padding(
              ...
              if (speciesList.isEmpty) {
                return Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.checklist_outlined, size: 48, color: Theme.of(context).colorScheme.surfaceDim),
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      GridView.count(
                        ...
                        children: <Widget>[
                          buildGridMenuItem(context, Icons.edit_outlined, S.current.details, () async {
                            ...
                            final editedSpecies = await Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder:
                                    (context) => EditSpeciesScreen(
                                      species: species,
                                      allowDuplicatedSpeciesNames: _allowsDuplicatedSpeciesNames,
                                      existingSpeciesNames: _existingSpeciesNamesExcluding(species),
                                    ),
                              ),
                            );
                          ...
                          if (species.isOutOfInventory)
                            buildGridMenuItem(context, Icons.inventory_outlined, S.current.addSpeciesToSample, () {
                              Navigator.of(context).pop();
                          ...
                          if (!species.isOutOfInventory)
                            buildGridMenuItem(
                              ...
                              S.of(context).removeSpeciesFromSample,
                              () async {
                                Navigator.of(context).pop();
                                _removeSpeciesFromSample(context, species);
                              },
                            ),
                          buildGridMenuItem(context, Icons.add_location_outlined, S.of(context).addPoi, () async {
                            Navigator.of(context).pop();
                          ...
                          buildGridMenuItem(context, Icons.delete_outlined, S.of(context).delete, () async {
                            Navigator.of(context).pop();
                      ...
                      GridView.count(
                        ...
                        children: <Widget>[
                          buildGridMenuItem(
                            ...
                            S.of(context).speciesAccumulationCurve,
                            () async {
                              Navigator.of(context).pop();
                              Navigator.push(
                                context,
                          ...
                          buildGridMenuItem(context, Icons.add_box_outlined, S.of(context).addSpecies, () async {
                            Navigator.of(context).pop();
                            _showAddSpeciesDialog(context, widget.speciesDao, widget.inventoryDao);
                          }),
                        ],
