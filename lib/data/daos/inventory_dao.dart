...
class InventoryDao {
  ...
  ///
  /// If an inventory with the same ID already exists, its main row is updated
    ...
    try {
      return await db.transaction((txn) async {
        ...
        for (final species in inventory.speciesList) {
          ...
          final int? speciesId = await txn.insert(
            'species',
          ...
          for (final poi in species.pois) {
            ...
            final int? poiId = await txn.insert(
              'pois',
        ...
        for (final vegetation in inventory.vegetationList) {
          ...
          final int? vegetationId = await txn.insert(
            'vegetation',
          ...
          if (vegetationId == null || vegetationId <= 0) {
        ...
        for (final weather in inventory.weatherList) {
          ...
          final int? weatherId = await txn.insert(
            'weather',
